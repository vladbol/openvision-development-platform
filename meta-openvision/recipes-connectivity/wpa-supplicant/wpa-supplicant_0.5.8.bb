...
TARGET_CFLAGS_append = " ${@bb.utils.contains("MACHINE_FEATURES", "madwifi", "-I${STAGING_INCDIR}/madwifi-ng", "", d)}"
...
export HASMADWIFI="${@bb.utils.contains('MACHINE_FEATURES', 'madwifi', 'yes', 'no', d)}"

do_configure () {
	install -m 0755 ${WORKDIR}/defconfig-openssl  .config
	if [ "x$HASMADWIFI" = "xyes" ]
	then
